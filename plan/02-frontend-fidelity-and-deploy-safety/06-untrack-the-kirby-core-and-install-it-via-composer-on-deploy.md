...
**Recommended model:** Strong → Claude Sonnet 5 — Mechanical, but it changes the deploy contract and touches git index state, so it needs care rather than cleverness.
...
>
> **Goal:** End the half-tracked app/kirby state so a core upgrade stops showing as thousands of phantom diffs.
>
...
>
> **Do this:**
> 1. Record the starting state for the build log: `git ls-files app/kirby | wc -l` (expected 551) and the Kirby version pinned in app/composer.json.
> 2. Run `git rm -r --cached app/kirby` to drop the tracked core from the index without touching the working tree. Confirm app/kirby/ still exists on disk and the site still boots.
> 3. Confirm .gitignore already covers app/kirby/ and app/vendor/ so nothing re-adds them; leave the ignore rules as they are.
> 4. In scripts/deploy.sh, add a Composer step before the existing npm build: run `composer install --no-dev --optimize-autoloader` in app/ and abort the deploy on a non-zero exit. Place it as a new numbered section ahead of the build so a broken dependency tree fails before anything is pushed.
> 5. Verify app/composer.json pins getkirby/cms to the 5.5 line so the deploy installs the version this site was tested against, not a future one.
> 6. Add an assertion after the Composer step that app/kirby/index.php exists, mirroring the existing app/assets/bundle check, so a silent Composer no-op cannot produce an empty core.
> 7. Run `bash scripts/deploy.sh --dry-run` and read the output. Confirm it reports pushing app/kirby content and nothing unexpected. Do not run a real deploy.
> 8. Update CLAUDE.md's setup commands and readme/QUICK_START.md to state that app/kirby is Composer-managed and never committed.
>
> **Acceptance criteria:**
> - `git ls-files app/kirby` returns nothing.
> - app/kirby/ still exists on disk and the site boots: PORT=8011 bash scripts/smoke.sh passes.
> - scripts/deploy.sh runs composer install --no-dev --optimize-autoloader before the npm build and aborts on failure.
> - scripts/deploy.sh asserts app/kirby/index.php exists after the Composer step.
> - `bash scripts/deploy.sh --dry-run` completes and its output was reviewed; no real deploy was run.
> - CLAUDE.md and readme/QUICK_START.md describe the Composer-managed core.
...
> - Mark Phase 6 done in `02-frontend-fidelity-and-deploy-safety/00-master.md`.

---

## Verification

- `git ls-files app/kirby`
- `PORT=8011 bash scripts/smoke.sh`
- `bash scripts/deploy.sh --dry-run`
- Read the deploy dry-run output and confirm nothing outside the expected trees is listed for deletion.
